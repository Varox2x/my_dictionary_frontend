        ...
        <S.Container>
            <S.List>
                {!isLoading && data && setsQueryDataToArray(data).map((set) => {
                    return < S.ListElement onClick={() => {
                        navigate(`set/${set.id}`)
                        dispatch({ type: ACTION_TYPES.CLOSE_MOBILE_MENU })
                    }} key={set.id}  > {set.name}</S.ListElement>
                }
                )}
                {hasNextPage && data && setsQueryDataToArray(data).length > 0 && <S.ListElement onMouseEnter={async () => await fetchNextPage()} key={"more"}>more...</S.ListElement>}
            </S.List>
        </S.Container>
    )
}
